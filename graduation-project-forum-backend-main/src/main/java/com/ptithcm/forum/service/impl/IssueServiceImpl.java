...
public class IssueServiceImpl implements IssueService {
  ...
  public ResponseDto<PageDto<IssueDto>> getPageOfIssues(IssueViewRequest issueViewRequest) {
    ...
    return new ResponseDto<>(PageDto.<IssueDto>builder()
        ...
        .data(issuePage.getContent().stream().map(issueMapping::convertIssueToIssueDto).collect(
            Collectors.toList()))
        .build());
  }
  ...
  public void getAndSaveIssuesFromPenpot() {
    ...
    String penpotToken = penpotTokens.get(0).getCodeName();
    HttpEntity request = getHttpEntity(penpotToken);
    String url = PENPOT_URL + PENPOT_TEAMS;
    List<PenpotTeamDto> penpotTeamDtos = getPenpotTeams(url, request);
    for (PenpotTeamDto penpotTeamDto : penpotTeamDtos) {
      url = PENPOT_URL + PENPOT_TEAM_RECENT_FILES + "?team-id=" + penpotTeamDto.getId();
      List<PenpotFileDto> penpotFileDtos = getPenpotTeamFiles(url, request);
      for (PenpotFileDto penpotFileDto : penpotFileDtos) {
        url = PENPOT_URL + PENPOT_COMMENT_THREADS + "?file-id=" + penpotFileDto.getId();
        ...
        for (PenpotCommentDto penpotCommentDto : penpotCommentDtos) {
          ...
          url = PENPOT_URL + PENPOT_FILE_OBJECT_THUMBNAILS + "?file-id=" + penpotFileDto.getId();
          ...
          Image image = uploadImage(PENPOT_URL + PENPOT_ASSETS_BY_ID + '/' + imageId);
          saveIssueAndHistory(penpotCommentDto, image);
        }
      }
    }
  }

  private void saveIssueAndHistory(PenpotCommentDto penpotCommentDto, Image image) {
  ...
  private Image uploadImage(String url) {
    ...
    try {
      minioUtil.putFileUrl(bucketName, url, fileName);
