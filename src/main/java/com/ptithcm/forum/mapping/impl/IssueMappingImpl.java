...
public class IssueMappingImpl implements IssueMapping {
  ...
  public IssueDto convertIssueToIssueDto(Issue issue) {
    ...
    issueDto.setPenpotPrototypeLink(issue.getPenpotPrototypeLink());
  ...
  @Override
  public IssueDto convertIssueToIssueDtoWithNumberPosts(Issue issue) {
    if (null == issue) {
      return null;
    }
    IssueDto issueDto = convertIssueToIssueDto(issue);
    issueDto.setNumberOfPosts((int) issue.getPosts().stream()
        .filter(p -> p.getStatus().equals(PostStatus.WAITING_APPROVE)).count());
    return issueDto;
  }
}
